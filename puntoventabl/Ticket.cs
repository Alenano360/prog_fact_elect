...
namespace PuntoVentaBL
{
    public class Ticket
    {
        ...
        private string _FacturaIdString;

        public string FacturaIdString
        {
            get { return _FacturaIdString; }
            set { _FacturaIdString = value; }
        }
        
        private decimal _CancelaApartado;

        public decimal CancelaApartado
        {
            get { return _CancelaApartado; }
            set { _CancelaApartado = value; }
        }
        ...
        {
            get { return _NuevoSaldo; }
            set { _NuevoSaldo = value; }
        }
	...
	public int Reimpresion
	{
        ...
        private decimal _Monto_efectivo;
        public decimal Monto_efectivo
        {
            get { return _Monto_efectivo; }
            set { _Monto_efectivo = value; }
        }

        private decimal _Monto_tarjeta;
        public decimal Monto_tarjeta
        {
            get { return _Monto_tarjeta; }
            set { _Monto_tarjeta = value; }
        }
        ...
        public void ObtieneInformacionGeneral()
        {
            try
            {
                ...
                _TipoCambio = Convert.ToDecimal(bus.TipoCambio);
        ...
        public void ImprimeCompra(object sender, PrintPageEventArgs e)
        {
            Offset = 0;
            ...
            //graphics.DrawString(_Nombre.ToUpper(), new Font("Merchant Copy Doublesize", 11),
            //    new SolidBrush(Color.Black), startX + 15, startY + Offset);
            //Offset = Offset + 20;

            //graphics.DrawString(_Dueno, new Font("Merchant Copy Doublesize", 7),
            //        new SolidBrush(Color.Black), startX + 40, startY + Offset);
            //Offset = Offset + 16;

            //graphics.DrawString("CED.:" + _Cedula, new Font("Merchant Copy Doublesize", 7),
            //        new SolidBrush(Color.Black), startX + 70, startY + Offset);
            //Offset = Offset + 16;

            //graphics.DrawString("PEREZ ZELEDON / TELEFONO: " + _Telefono, new Font("Merchant Copy Doublesize", 7),
            //        new SolidBrush(Color.Black), startX, startY + Offset);
            //Offset = Offset + 16;

            //graphics.DrawString("250MTS SUR TRIBUNALES DE JUSTICIA", new Font("Merchant Copy Doublesize", 7),
            //        new SolidBrush(Color.Black), startX, startY + Offset);
            //Offset = Offset + 16;

            //measureString = "FRENTE A TRACOPA";
            ...
            graphics.DrawString("Número de comprobante: " + _FacturaIdString , new Font("Merchant Copy Doublesize", 7),//+" Compra"
                    new SolidBrush(Color.Black), startX, startY + Offset);
            Offset = Offset + 16;
       ...
       new SolidBrush(Color.Black), Convert.ToInt32(170), startY + Offset);
            ...
            if (_Desc_Aplicado > 0)
            {
                ...
                //graphics.DrawString("TOTAL $: " + Convert.ToDecimal(_TotalFactura / Convert.ToDecimal(_TipoCambio)).ToString("F"), new Font("Merchant Copy Doublesize", 9),
                //       new SolidBrush(Color.Black), startX, startY + Offset);
                //Offset = Offset + 20;
            }
            else
        ...
        public void ImprimeApartado(object sender, PrintPageEventArgs e)
        {
            Offset = 0;
            string Impresora = string.Empty;
            ...
            graphics.DrawString("Número de apartado: " + _FacturaId + "   " + "Apartado", new Font("Merchant Copy Doublesize", 7),
                    new SolidBrush(Color.Black), startX, startY + Offset);
            Offset = Offset + 16;
       ...
       new SolidBrush(Color.Black), Convert.ToInt32(170), startY + Offset);
            ...
            else
            {
                ...
                //graphics.DrawString("TOTAL $: " + Convert.ToDecimal(_TotalFactura / Convert.ToDecimal(_TipoCambio)), new Font("Merchant Copy Doublesize", 9),
                //       new SolidBrush(Color.Black), startX, startY + Offset);
                //Offset = Offset + 20;
            }
            ...
            graphics.DrawString("Abono : "+_MontoAbono.ToString("##,#0.#0") , new Font("Merchant Copy Doublesize", 8),
                new SolidBrush(Color.Black), startX, startY + Offset);
            Offset = Offset + 20;

            if (_CancelaApartado==0)
            {
                graphics.DrawString("Saldo : " + Convert.ToDecimal(_TotalFactura - _MontoAbono).ToString("##,#0.#0"), new Font("Merchant Copy Doublesize", 8),
                    new SolidBrush(Color.Black), startX, startY + Offset);
                Offset = Offset + 20;                
            }
            else
            {
                graphics.DrawString("Saldo : " + Convert.ToDecimal(_NuevoSaldo).ToString("##,#0.#0"), new Font("Merchant Copy Doublesize", 8),
                    new SolidBrush(Color.Black), startX, startY + Offset);
                Offset = Offset + 20;
            }
        ...
        {
            if (ImpresionCompra==1)
            {
                ImprimeCompra(sender, e);
                return;
            }

            if (Apartado == 1)
            {
                ImprimeApartado(sender, e);
                return;
            }
            ...
            if (_Proforma == 1)
            {
                graphics.DrawString("Número de Proforma: " + _FacturaId, new Font("Merchant Copy Doublesize", 7),
                        new SolidBrush(Color.Black), startX, startY + Offset);
                Offset = Offset + 16;
            }
            else if (_Prefactura == 1)
            {
                graphics.DrawString("Número de Prefactura: " + _FacturaId, new Font("Merchant Copy Doublesize", 7),
                        new SolidBrush(Color.Black), startX, startY + Offset);
                Offset = Offset + 16;
            }
            else if (_NotaCredito == 1)
            {
                graphics.DrawString("Número de Nota de Crédito: " + _FacturaId, new Font("Merchant Copy Doublesize", 7),
                        new SolidBrush(Color.Black), startX, startY + Offset);
                Offset = Offset + 16;

                graphics.DrawString("Factura asociada: " + _FacturaIdString, new Font("Merchant Copy Doublesize", 7),
                        new SolidBrush(Color.Black), startX, startY + Offset);
                Offset = Offset + 16;
            }
            else
       ...
       new SolidBrush(Color.Black), Convert.ToInt32(170), startY + Offset);
            ...
            if ( _Prefactura != 1 && _NotaCredito != 1)//pone pie a proforma y a factura
            {
                ...
                if (_Recibido>0)
                {
                    ...
                    graphics.DrawString("TOTAL: " + Convert.ToDecimal(_TotalFactura).ToString("F"), new Font("Merchant Copy Doublesize", 10),
                            new SolidBrush(Color.Black), startX, startY + Offset);
                    Offset = Offset + 20;

                    if (_Monto_efectivo > 0 && _Monto_tarjeta <= 0)
                    {
                        graphics.DrawString("EFECTIVO: " + Convert.ToDecimal(_Recibido).ToString("F"), new Font("Merchant Copy Doublesize", 10),
                                new SolidBrush(Color.Black), startX, startY + Offset);
                        Offset = Offset + 20;
                    }
                    else if (_Monto_efectivo > 0 && _Monto_tarjeta > 0)
                    {
                        graphics.DrawString("EFECTIVO: " + Convert.ToDecimal(_Monto_efectivo).ToString("F"), new Font("Merchant Copy Doublesize", 10),
                                new SolidBrush(Color.Black), startX, startY + Offset);
                        Offset = Offset + 20;

                        graphics.DrawString("TARJETA: " + Convert.ToDecimal(_Monto_tarjeta).ToString("F"), new Font("Merchant Copy Doublesize", 10),
                               new SolidBrush(Color.Black), startX, startY + Offset);
                        Offset = Offset + 20;
                    }

                    else if (_Monto_tarjeta > 0 && _Monto_efectivo <= 0)
                    {
                        graphics.DrawString("TARJETA: " + Convert.ToDecimal(_Monto_tarjeta).ToString("F"), new Font("Merchant Copy Doublesize", 10),
                                new SolidBrush(Color.Black), startX, startY + Offset);
                        Offset = Offset + 20;
