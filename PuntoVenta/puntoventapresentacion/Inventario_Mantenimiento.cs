...
namespace PuntoVentaPresentacion
{
    public partial class Inventario_Mantenimiento : Form
    {
        ...
        PuntoVentaBL.IVA mod = new PuntoVentaBL.IVA();
        ...
        private void Inventario_Agregar_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Accion==0)//Agregar articulo
                {
                    ...
                    //this.txtIVPorc.Text = Convert.ToInt32(this.Cb_IVA.SelectionStart).ToString();
                }
                if (Accion==1)//Modifica Articulo
                {
                    this.Cb_IVA.SelectedValue = IV;
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                char[] values = ((System.Windows.Forms.ComboBox)(this.Cb_IVA)).Text.ToCharArray();
                int numIndex = Array.IndexOf(values, '%');
                if (numIndex >= 0)
                    values = values.Where((val, idx) => idx != numIndex).ToArray();
                string cbiva = new string(values);
                decimal iv = Convert.ToDecimal(cbiva) / 100;
                ...
                if (Login.RolId.ToString() == "1")//solo admin puede verif (this.cmbUnidadMedida.Text != "Unidad(es)")
                {
                    ...
                    this.objInventario.IV = Convert.ToInt32(this.Cb_IVA.SelectedValue);
                    this.objInventario.Gram = Convert.ToDecimal(txtgram.Text);
                    //this.objInventario.precioxGram = Decimal.Round(Convert.ToDecimal(txtprecioxgram.Text),2);
                    //if (this.chkGravado1.Checked)
                    //if (this.Cb_IVA.Text != "0%")
                    //{
                    ...
                    this.objInventario.MontoIV = Convert.ToDecimal(this.txtPrecioImpuesto.Text) - Convert.ToDecimal(this.txtPrecio1.Text);
        ...
        private void CalculaPrecioImpuesto()
        {
            try
            {
                ...
                if (this.Cb_IVA.Text != "0%")
                {
                    ...
                    string cbiva = new string(values);
                    decimal iv = Convert.ToDecimal(cbiva) / 100;
                    decimal temp = precio + (precio * iv);
                    //decimal fin = Decimal.Round(temp,0);
                    this.txtPrecioImpuesto.Text = temp.ToString("F");
                }
                else
                {
                    decimal precio = Convert.ToDecimal(this.txtPrecio1.Text);
...
#region CodigoAnterior
        ...
        private void CalculaPorcUtilidad1()
        {
            try
            {
                ...
                //ELIMINA EL % DEL TEXTO DEL IVA SELECT
                char[] values = ((System.Windows.Forms.ComboBox)(this.Cb_IVA)).Text.ToCharArray();
                int numIndex = Array.IndexOf(values, '[');
                if (numIndex >= 0)
                    values = values.Where((val, idx) => idx != numIndex).ToArray();
                numIndex = Array.IndexOf(values, '%');
                if (numIndex >= 0)
                    values = values.Where((val, idx) => idx != numIndex).ToArray();
                numIndex = Array.IndexOf(values, ']');
                if (numIndex >= 0)
                    values = values.Where((val, idx) => idx != numIndex).ToArray();
                string cbiva = new string(values).Replace(' ', '0');
                decimal iv = Convert.ToDecimal(cbiva) / 100;
                //DEFINIMOS VARIABLES PARA EL CALCULO
                decimal precio = Convert.ToDecimal(this.txtPrecio1.Text);
                //calcula la ganancia
                decimal ganancia = 0;
                if(Convert.ToDecimal(this.txtUtilPorc1.Text) > 0)
                    ganancia = precio * (Convert.ToDecimal(this.txtUtilPorc1.Text) / 100);
                //calcula el impuesto
                decimal impuesto = (precio + ganancia) * iv;
                //calcula el precio  final
                decimal fin = Math.Round(precio + impuesto + ganancia);
                int roundFinal = ((Decimal.ToInt32(fin) + 5)/ 10) *10;
                //((orig + 5) / 10) * 10;
                this.txtPrecioImpuesto.Text = Math.Round(impuesto+precio, 0).ToString("F");
                this.txtPrecioFinal.Text = roundFinal.ToString("F");
...
//                if (this.Cb_IVA2.Text != "0%")
//                {
//                    decimal precio = Convert.ToDecimal(this.txtPrecio2.Text);
//                    decimal precioimpuesto = Convert.ToDecimal(this.txtPrecioImpuesto2.Text);
//                    char[] values = ((System.Windows.Forms.ComboBox)(this.Cb_IVA2)).Text.ToCharArray();
//                    int numIndex = Array.IndexOf(values, '%');
//                    if (numIndex >= 0)
//                        values = values.Where((val, idx) => idx != numIndex).ToArray();
//                    string cbiva = new string(values);
...
//                else
//                {
//                    decimal precio = Convert.ToDecimal(this.txtPrecioImpuesto2.Text);
//                    decimal fin = precio;
//                    this.txtPrecioImpuesto2.Text = fin.ToString("F");
...
//        }
        ...
        private void txtgram_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                gram = Convert.ToDecimal(this.txtgram.Text);
                  if ((Convert.ToDecimal(txtPrecioImpuesto.Text) > 0)&&(Convert.ToDecimal(txtgram.Text)>0))
            {
                txtprecioxgram.Text = (Convert.ToDecimal(txtPrecioImpuesto.Text) / Convert.ToDecimal(txtgram.Text)).ToString();
            }

                  if ((Convert.ToDecimal(txtPrecioImpuesto2.Text) > 0) && (Convert.ToDecimal(txtgram.Text) > 0))
                  {
                      txtprecioxgram2.Text = (Convert.ToDecimal(txtPrecioImpuesto2.Text) / Convert.ToDecimal(txtgram.Text)).ToString();
                  }
            }
            catch (Exception)
            {
        ...
        //private void gbPrecios_Enter(object sender, EventArgs e)
        //{

        //}

        private void Cb_IVA_SelectedValueChanged(object sender, EventArgs e)
        {
            try
            {
                char[] values = ((System.Windows.Forms.ComboBox)(sender)).Text.ToCharArray();
                int numIndex = Array.IndexOf(values, '[');
                if (numIndex >= 0)
                    values = values.Where((val, idx) => idx != numIndex).ToArray();
                numIndex = Array.IndexOf(values, '%');
                ...
                if (numIndex >= 0)
                    values = values.Where((val, idx) => idx != numIndex).ToArray();
                string p_iva = new string(values).Replace(' ', '0');
                this.txtIVPorc.Text = p_iva;
                CalculaPorcUtilidad1();
                //try
                //{
                //    if (Convert.ToDecimal(this.txtPrecio1.Text) == 0)
                ...
                //        decimal precio = Convert.ToDecimal(this.txtPrecio1.Text);
                //        decimal precioimpuesto = Convert.ToDecimal(this.txtPrecioImpuesto.Text);
                //        decimal iv = Convert.ToDecimal(p_iva) / 100;
                //        decimal temp = precio + (precio * iv);
                //        decimal fin = temp;
                //        this.txtPrecioImpuesto.Text = fin.ToString("F");
                //    }
                ...
                //        decimal temp = precio;
                //        decimal fin = temp;
                //        this.txtPrecioImpuesto.Text = this.txtPrecio1.Text;
                //    }
                //}
                //catch (Exception ex)
                //{
        ...
        //private void Cb_IVA2_SelectedValueChanged(object sender, EventArgs e)
        //{
        //    try
        //    {
        //        char[] values = ((System.Windows.Forms.ComboBox)(sender)).Text.ToCharArray();
        ...
        //            values = values.Where((val, idx) => idx != numIndex).ToArray();
        //        string p_iva = new string(values).Replace(' ', '0');
        //        p_iva = new string(values).Replace(',', '.');
        //        //this.txtIVPorc.Text = p_iva;
        ...
        //                decimal temp = precio;
        //                decimal fin = temp;
        //                this.txtPrecioImpuesto2.Text = this.txtPrecio2.Text;
        //            }
        //        }
        //        catch (Exception ex)
        //        {
