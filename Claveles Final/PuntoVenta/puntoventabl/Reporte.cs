...
namespace PuntoVentaBL
{
    public class Reporte
    {
        ...
        private string t1, t2, t3, t4 = string.Empty;

        // The class constructor


        public Reporte(DataGridView aDataGridView, PrintDocument aPrintDocument, bool CenterOnPage, bool WithTitle, string aTitleText, Font aTitleFont, Color aTitleColor, bool WithPaging)
        {
            ...
            //aPrintDocument.PrinterSettings.PrinterName = "Microsoft XPS Document Writer";
            ...
            //LeftMargin = 2;//ThePrintDocument.DefaultPageSettings.Margins.Left;
            //TopMargin = 2;//ThePrintDocument.DefaultPageSettings.Margins.Top;
            //RightMargin = 2;//ThePrintDocument.DefaultPageSettings.Margins.Right;
            //BottomMargin = 2;//ThePrintDocument.DefaultPageSettings.Margins.Bottom;

            // First, the current row to be printed is the first row in the DataGridView control
            CurrentRow = 0;
        }

        public Reporte(DataGridView aDataGridView, string text1, string text2, string text3, string text4, PrintDocument aPrintDocument, bool CenterOnPage, bool WithTitle, string aTitleText, Font aTitleFont, Color aTitleColor, bool WithPaging)
        {
            TheDataGridView = aDataGridView;
            ThePrintDocument = aPrintDocument;
            IsCenterOnPage = CenterOnPage;
        ...
        private void DrawHeader(Graphics g)
        {
            ...
            if (IsWithPaging)
            {
               ...
               //RectangleF PageStringRectangle = new RectangleF((float)LeftMargin, CurrentY, (float)PageWidth - (float)RightMargin - (float)LeftMargin, g.MeasureString(PageString, PageStringFont).Height);
                RectangleF PageStringRectangle = new RectangleF((float)LeftMargin, CurrentY, (float)PageWidth - (float)RightMargin - (float)LeftMargin, (float)PageHeight);
        ...
        private bool DrawRows(Graphics g)
        {
            ...
            if (t1!=null)
            {
                g.DrawString(t1.ToUpper(), TheDataGridView.DefaultCellStyle.Font,
                    new SolidBrush(Color.Black), (float)der, CurrentY);
                CurrentY = CurrentY + 20;                
            }
            if (t2 != null)
            {
                g.DrawString(t2.ToUpper(), TheDataGridView.DefaultCellStyle.Font,
                 new SolidBrush(Color.Black), (float)der, CurrentY);
                CurrentY = CurrentY + 20;
            }
            if (t3 != null)
            {
                g.DrawString(t3.ToUpper(), TheDataGridView.DefaultCellStyle.Font,
                    new SolidBrush(Color.Black), (float)der, CurrentY);
                CurrentY = CurrentY + 20;
            }
            if (t4 != null)
            {
                g.DrawString(t4.ToUpper(), TheDataGridView.DefaultCellStyle.Font,
                    new SolidBrush(Color.Black), (float)der, CurrentY);
                CurrentY = CurrentY + 20;
            }
